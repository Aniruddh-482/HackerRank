...
assign the memory address of val to pointer p. To access the content of the memory to which the pointer points, prepend it
with a *. For example, *p will return the value reflected by val and any modification to it will be reflected at the source (val).
...
void update(int *a,int *b) {
    ...
    *a = *a + *b;
    *b = abs(*a - 2*(*b));    // In the C Programming Language, the abs function returns the absolute value of an integer.
}
